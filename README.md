...
Fünf Ansichten:
- **Karte:** Gemeinde- oder Kantonskarte, interaktiv nach **exklusiver Kategorie (1–10)** eingefärbt. Rechts wählst du **einen oder mehrere** der 10 Instrument-Typen (mit Gebiets-Zähler); Umschalter **farbbestimmend** (nur in Kraft + bindend + Tier-1/2) ↔ **alle Instrumente** (inkl. Förderung/Vollzug/Pendentes). Bei mehreren gewählten Typen gilt der stärkste. Gebiete mit erfassten, aber (im aktuellen Modus) **nicht farbbestimmenden** Instrumenten (z. B. Förderung oder noch nicht in Kraft getretene Quoten) werden **schraffiert** — so sind sie von wirklich leeren Gebieten unterscheidbar (Klick zeigt die Instrumente). Hover/Klick zeigt Instrumente mit Quelle; **SVG-Export**.
